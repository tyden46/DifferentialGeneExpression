...
png(filename="histPlot.png", width=2000, height=2000)
...
ggplot(allDiff) + geom_histogram(aes(x=log2FoldChange))+
  ...
  facet_grid(~chromosome_name_f) +
  theme_grey() +
  theme(text = element_text(size = 20)) +
  scale_colour_brewer(palette = colorRampPalette(brewer.pal(11, "Spectral"))(length(chromosomes)))
